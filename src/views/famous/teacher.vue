...
<script>
    import { getTeacherInfo, getSchoolGoodsList, getGoodsStatisticsBySchoolId } from '../../api/api'
    ...
    export default {
        ...
        methods: {
            load(calltime) {
                ...
                getTeacherInfo(this.userid).then((res) => {
                    if (res.data.serverResult.resultCode == "200") {
                        var teacherInfo = res.data.responseEntity;
                        this.username = teacherInfo.realname;
                        this.count = teacherInfo.goodscount;
                    } else {
                ...
                this.steps = [
                    ...
                    {
                        title:'名师资源',
                        url:'/famous/teacher/' + this.userid
                    }
                ];
            ...
            setSession() {
                localStorage.setItem('famous-teacher-navigation', JSON.stringify(this.selectedItems));
            },
            onChangePage(page) {
                this.page = page;
                this.loadData(new Date().getTime());
        ...
        mounted() {
            let teacherNavi = localStorage.getItem('famous-teacher-navigation');
            if (teacherNavi != null && teacherNavi != '') {
                teacherNavi = JSON.parse(teacherNavi);
                if (teacherNavi.schoolsectionid == '' || 
                    teacherNavi.schoolsectionid == undefined) {
                    teacherNavi.schoolsectionid = 'PRIMARY_SCHOOL'; // 小学
                }
                this.$refs.grade.setNavigation(teacherNavi);
            } else {
                teacherNavi = {
