        ...
        <div class="ecrw-content-hot-det">
            <ul>
                <li class="ecrw-content-hot-li" :class="ECRUtil.resourceType(item.contenttype)" v-for="(item,idx) in items" :key="idx" :data="item">
                    ...
                    <a :href="'resource/' + item.contentid" class="ecrw-hot-href" target="_blank">{{ item.name }}</a>
                </li>
            </ul>
        </div>
    </section>
...
<script>
    import { getCollectedResourceList } from '../../../api/api';
    ...
    export default {
        ...
        methods: {
            load() {
                ...
                getCollectedResourceList(params).then((res) => {
